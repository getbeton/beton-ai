    ...
    try {
      ...
      for (let page = 1; page <= totalPages; page++) {
        try {
          ...
          // Get the actual job info with correct totals for accurate progress calculation
          const currentJobInfo = await BulkDownloadService.getJobInfo(jobId);
          if (!currentJobInfo) {
            throw new Error('Failed to get job info for progress update');
          }

          // Update Bull Queue progress AFTER data insertion using ACTUAL progress
          job.progress(currentJobInfo.progress.percentage);
          console.log(`Page ${page}/${totalPages}: Bull queue progress updated to ${currentJobInfo.progress.percentage}% (${totalProcessed} of ${currentJobInfo.progress.totalEstimated} records)`);

          // Send WebSocket progress update AFTER data insertion with ACCURATE progress
          WebSocketService.sendJobProgress(userId, currentJobInfo);
          console.log(`Page ${page}/${totalPages}: WebSocket progress sent - ${currentJobInfo.progress.percentage}% (${totalProcessed}/${currentJobInfo.progress.totalEstimated} records)`);
      ...
      // Mark job as completed ONLY after verification
      const completedJob = await prisma.bulkDownloadJob.update({
        where: { id: jobId },
        data: { 
          status: BULK_DOWNLOAD_CONFIG.JOB_STATUS.COMPLETED,
          completedAt: new Date(),
          totalProcessed: actualRowCount // Use verified count
        }
      });
      ...
      console.log(`Bulk download job ${jobId} completed successfully. Verified ${actualRowCount} records in database.`);

      // Send completion notification via WebSocket ONLY after verification
      const completedJobInfo = await BulkDownloadService.getJobInfo(jobId);
      if (completedJobInfo) {
        console.log(`Sending completion notification: ${completedJobInfo.progress.percentage}% (${completedJobInfo.progress.processedRecords}/${completedJobInfo.progress.totalEstimated})`);
